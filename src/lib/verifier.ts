...
export class MessageVerifier {
   ...
   */
  private cryptoKey = createHash('sha256')
    .update(this.secret)
    .digest();
  ...
  public sign(value: any, expiresAt?: string | number, purpose?: string) {
    ...
    const encoded = base64.urlEncode(
      new MessageBuilder().build(value, expiresAt, purpose)
    );
    return `${encoded}${this.separator}${new Hmac(this.cryptoKey).generate(
      encoded
    )}`;
  }
